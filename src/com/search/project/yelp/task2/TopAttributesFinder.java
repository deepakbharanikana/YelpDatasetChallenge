...
public class TopAttributesFinder {
	...
	public static void main(String gg[]) {
		 ...
		 * 
		 * Import restaurant business file and reviews.json in MongoDB Now, we
		 * have "restaurants and "reviewCollection" in DB
		 * 
		 * Execute below command to create "BusinessReviewGroupCollection
		 * db.reviewCollection
		 * .aggregate([{$group:{_id:"$business_id",reviews:{$push:
		 * {text:"$text"}
		 * }}},{$out:"BusinessReviewGroupCollection"}],{allowDiskUse:true})
		 * 
		 * The above command will group the reviews according to business_ids.
		 * 
		 * Join the BusinessReviewGroupCollection and restaurants collection on
		 * their business_ids. This creates a hierarchical model that comprises
		 * of each doc having business_id,city, categories, business_name and
		 * their reviews
		 * 
		 * createBusinessReviewCity() is executed for above process.
		 * 
		 * Now, we create train and test collection on basis of city_category
		 * information. Each doc will have "city_category" as id and business
		 * reviews as their value.
		 * 
		 ...
		 * System
		 * .out.println("Displaying Related words for "+queryFeature+" :"); for
		 * (String relWord : relatedWords) { System.out.println(relWord); }
		 */
		try {
	...
	/**
	 * Creates a business file that has businesses related to restaurant
	 * category
	 */
	private static void createRestaurantBusinessFile() {
		...
		try {
			...
			FileReader inputFile = new FileReader(Constants.datasetLocation
					+ "//" + Constants.businessFile);
			...
			File file = new File(Constants.datasetLocation + "//"
					+ Constants.restaurantBusinessFile);
			...
			while ((line = bufferReader.readLine()) != null) {
				...
				JSONArray catArray = (JSONArray) jsonObj
						.get(Constants.CATEGORIES_KEY);
			...
			restaurantObj.put(restaurants_category.toLowerCase(),
					businessObjArray);
	...
	public static List<String> generateQueryForFeature(String featureWord) {
		...
		if (synsets.length > 0) {
			System.out.println("The following synsets contain '" + featureWord
			...
			for (int i = 0; i < synsets.length; i++) {
				...
				for (int j = 0; j < usageExamples.length; j++) {
					...
					relWordsSet.addAll(Arrays.asList(usageExamples[j]
							.split(" ")));
				}
		...
		/*
		 * Iterator<String> itr = relWordsSet.iterator(); while(itr.hasNext()){
	...
	private static void calculateAttributeScores() throws IOException,
			ParseException {

		Map<String, String> queryMap = new HashMap<String, String>();
		queryMap.put("Wheelchair accessible", "wheelchair accessible handicap");
		queryMap.put("Happy Hour",
				"drinks reduced  hour decreased lower discount");
		queryMap.put("Caters", "banquets host cater conference feast");
		queryMap.put("Smoking",
				"tobacco cigar hookah cigarette pipe fire lighter");
		queryMap.put("Desserts", "dessert icecream sweet cake confection");
		queryMap.put("Casual Ambience",
				"casual natural style interesting elegant Casual ambience informal");
		queryMap.put("Take-Out", "take-out to-go take-away pickup carryout");
		queryMap.put("Price Range",
				"price affordable expensive cost cheap worth bucks value purchase bill fare");
		queryMap.put("Alcohol",
				"liquor alcohol intoxicant brew alcoholic beverage bar pubs lounge");
		queryMap.put("Music",
				"jazz music rock melody  pop club dance Listening Singer band Beats ");
		queryMap.put("Delivery",
				"delivery to-go online-order pick-up quick ontime home-delivery parcel");
		// queryMap.put("Good For Kids","kid play baby child colorful game young son daughter boy girl");
		queryMap.put("Waiter Service",
				"service maintenance clean serving waiter early friendly horrible speed slow");
		queryMap.put("Accepts credit cards",
				"credit card debit payment cash visa mastercard");
		queryMap.put("Wi-Fi",
				"wifi wi-fi internet signal wireless password network distant");
		queryMap.put("Drive-Thru", "drive-through drive-thru");
		queryMap.put(
				"Takes Reservation",
				"booking reserve advance appointment waiting delay vacancy house-full occupied accommodation");
		queryMap.put("Parking",
				"parking vehicle car bike space lot convenient garage");
		...
		for (Entry<String, String> queryEntry : queryMap.entrySet()) {
			...
			String index = Constants.trainIndexDir
					+ "\\BusinessReviewTipCity\\" + cityName;
			...
			/*
			 * ScoreDoc[] hits = results.scoreDocs; for (int i = 0; i <
			 * hits.length; i++) { Document doc = searcher.doc(hits[i].doc);
	...
	/**
	 * @param : Map<String, Double> unsortMap : a Map which has to be sorted on
	 *        keys
